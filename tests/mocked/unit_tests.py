...
class add_user_testCase(unittest.TestCase):
    ...
    def test_success(self):
        for test in self.success_test_params:
            initial_person = models.Player(username=INITIAL_USERNAME,
                                           score=100)
            ...
            with patch('app.DB.session.add', self.mocked_db_session_add):
                with patch('app.DB.session.commit',
                    ...
                    with patch('app.DB.session.query') as mocked_query:
                        mocked_query(
                            Player).all = self.mocked_db_session_query_all
...
class pull_leaderboard_testCase(unittest.TestCase):
    def setUp(self):
        ...
        self.success_test_params = [{
            ...
            KEY_EXPECTED: {
                'players': [INITIAL_USERNAME],
                'scores': [100]
            }
        }, {
            DATABASE:
            [self.first_person, self.second_person, self.third_person],
            KEY_EXPECTED: {
                'players': [INITIAL_USERNAME, 'Guest01', ''],
                'scores': [100, 0, -3]
            }
        }, {
            DATABASE: [],
    ...
    def test_success(self):
        for test in self.success_test_params:
            ...
            with patch('app.DB.session.query') as mocked_query:
                mocked_query(Player).order_by(
                    Player.score.desc()).all = self.mocked_db_session_query_all
